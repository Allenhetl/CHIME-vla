...
- [C3][C4] 写入瞬间对 γ_*_t 加 sg —— 阻断 L_main 训写入门(HC-3)
- [C11] PRH 输入 m_t 与 [C8] 的 query 投影矩阵都 sg —— 阻断 L_PRH 污染 [C1]
- [C10] 输出 γ̂ 在喂入 [C5] 前 sg —— γ̂ 是 target 不是可学量
- [C12] 经 frozen [C9] —— 自然 sg
```
...
**接口契约**:
- M_sem ∈ R^{K_s×d_s},K_s=64,d_s=256 [标定值]
- Episode 间清空(放弃跨 episode 学习,见 §H Trade-off 2)

**内部 operator**:K_s 个 slot,key k_i 在 episode 开始时随机初始化 + frozen,value v_i 通过 [C4] delta-rule 累加。容量满时 LRU 丢弃(见 [C4] 步骤 5)。
...
λ_1=0.3, λ_2=0.5, λ_3=0.1, λ_4=0.2 [全部标定值]。
...
> **重要**:本节是早期 H100 假设下的 MVP 描述,**已被 §0.7.4(基于 6×A800 训练 + 4090 48G 推理硬件实际)替换**。读者应当以 §0.7.4 为准。本节保留作为 H100 假设下的对照版,以便理解硬件锁定如何影响 MVP 配置(主要差异:[C1] 从 ViT-L 降为 ViT-B、[C9] 加 1-step 蒸馏、[C5] ψ 从 1-layer Transformer 改 GRU、推理 INT8、batch 从 32 降到 24)。

**砍掉**(H100 版,§0.7.4 一致):[C10][C12][C13]、L_HCS、L_CSM、L_GC;[C6] 多分辨率简化为单分辨率 16³;benchmark 收敛到 LIBERO-Long + CALVIN ABCD→D。

**保留**(H100 版):[C1 ViT-L][C2][C3 简化][C4][C5 仅 self-supervised prediction error,1-layer Transformer ψ][C6 16³][C7 timestamp eviction 替代 CSM-LRU][C8][C9][C11] + L_main + L_PRH + L_aux。

**注意**:MVP 配置下 disable [C13] 与 L_HCS 同时意味着 §3.3"跨百步信用分配"在 MVP 中**完全不被回应**,与 §F-选择 3 fall-back 一致。Publishable claim 退化为"打 HC-3 + §3.2 + §3.4 三条"。

**最小可发表 paper claim**:"Event-segmentation prediction error 驱动的双通道 delta-rule 不可变 memory + MERLIN 风格 predictive read,在 CALVIN ABCD→D + LIBERO-Long 上 SR 比 OpenVLA + history baseline 高 X%。"——workshop paper 级别,工程可控。**§0.7.4 在此之上加"消费级硬件 GPU 可部署"作为 contribution**。

### §I.3 6-month full 版本 milestone

- **M1 (week 4)**:E1 完成。BridgeV2 100-200 traj 手标(1-2 人周额外)+ [C10] J^{(Δ)} + IoU 报告。**门槛:IoU @ 0.3 ≥ 0.4**。失败 → fallback 到 MVP(§0.7.4 配置)。
